...
Fac_BioMetric_Pheno = BioMetric_Pheno%>%
  select(Sub, AgeAtTestingCalculated, Sex, FA_A, FA_B,TwinType)%>%
  rename(Age = "AgeAtTestingCalculated",
         avgP_FAres_1 = "FA_A",
         avgP_FAres_2 = "FA_B")%>%
  as.data.frame()
# ----------------------------------------------------------------------------------------------------------------------
vars <- 'avgP_FAres' # list of variables names
...
load(sprintf("%s/%s/02_Sutherland_2020/09_1_SAT_ebres_faces_Sutherland2020.Rdata", wdOA,wdOA_output))
#compare models
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitACE,fitAE,fitCE,fitE, file = sprintf("%s/%s/02_Sutherland_2020/13_1_ADE_ebres_faces_Sutherland2020.Rdata",wdOA,wdOA_output))
write_csv(cbind(modelComparison,ACEvc),sprintf("%s/%s/02_Sutherland_2020/13_1_ADE_ebres_faces_vc_modelComparison_val.csv",wdOA,wdOA_output))
write_csv(as.data.frame(AECIests),sprintf("%s/%s/02_Sutherland_2020/13_1_ADE_ebres_faces_vc_bestModel_val.csv",wdOA,wdOA_output))
