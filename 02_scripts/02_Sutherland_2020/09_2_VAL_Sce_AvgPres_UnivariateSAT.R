...
#
#Description:
# Twin Univariate Saturated model to estimate means and (co)variances (age and sex) across 2 groups (MZss and DZss) for residualized data
...
#load dataFrames:
BioMetric_Pheno  = read_csv(sprintf("%s/%s/02_Sutherland_2020/05_twin_ebres.csv", wdOA,wdOA_output))

#load functions:
source(sprintf("%s/%s/functions/miFunctions.R", wdOA,wdOA_scripts))
Sce_BioMetric_Pheno = BioMetric_Pheno%>%
  dplyr::select(Sub, AgeAtTestingCalculated, Sex, SC_A,SC_B,TwinType)%>%
...
vars <- 'avgP_SCres' # list of variables names
...
# Set Starting Values
svBeAge <--0.0001 # start value for regressions
svBeSex <--0.01 # start value for regressions
svMe <- 0.1 # start value for means
svVa <- .5 # start value for variance
# lbVa <- .05 # lower bound for variance
# PREPARE MODEL----------------------------------------------------------------------------------------------------------------------
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitSAT,fitCOVsex,fitCOVsex,fitEMO,fitEMO,fitEMVO,fitEMVZ, file = sprintf("%s/%s/02_Sutherland_2020/09_2_SAT_ebres_scenes_Sutherland2020.Rdata",wdOA,wdOA_output))
write_csv(fitSATdescriptives,sprintf("%s/%s/02_Sutherland_2020/09_2_SAT_ebres_scenes_parameters_val.csv",wdOA,wdOA_output))
write_csv(modelComparison,sprintf("%s/%s/02_Sutherland_2020/09_2_SAT_ebres_scenes_modelComparison_val.csv",wdOA,wdOA_output))
