...
#
#
#Description: repeat procedure in highlighted in 02_mlm_twin1 (but for twin 2)
#Program: xxx ------------------------------------------------------------------------------------------------------------------------------
...
wdNOA_ImageOutput = "05_images/image"

#load dataFrames:
Fac_MLM_TwinLong  = read_csv(sprintf("%s/%s/02_Sutherland_2020/01_MLM_Fac_Twin2_Sutherland2020.csv", wdOA,wdOA_output))
Sce_MLM_TwinLong  = read_csv(sprintf("%s/%s/02_Sutherland_2020/01_MLM_Sce_Twin2_Sutherland2020.csv", wdOA,wdOA_output))
#load functions:
source(sprintf("%s/%s/functions/VCA.R", wdOA,wdOA_scripts))

#parameters:
#simulations
NSim = 1 #CI are not needed here
...
write_csv(VCAsummary,sprintf("%s/%s/02_Sutherland_2020/02_sup_S3_VCA_twin2_Sutherland2020.csv",wdOA,wdOA_output))
save(MLMsummary, file = sprintf("%s/%s/02_Sutherland_2020/02_sup_S3_MLM_twin2_Sutherland2020.RData",wdOA,wdOA_output))