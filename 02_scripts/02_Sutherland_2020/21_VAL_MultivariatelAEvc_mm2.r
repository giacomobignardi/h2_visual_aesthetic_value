...
#
#Description:
# Twin Multivariate model to estimate means and (co)variances across multiple groups
...
#load dataFrames:
BioMetric_Pheno  = read_csv(sprintf("%s/%s/02_Sutherland_2020/05_twin_tt_multivariate.csv", wdOA,wdOA_output))
#load fit sat to compare with saturated model
load(sprintf("%s/%s/02_Sutherland_2020/17_SAT_tt_multivariate_Sutherland2020.RData", wdOA,wdOA_output))
...
modelComparison = rbind(
  ...
  mxCompare( fitACE, nested <- list(fitACE, nested <- list(fitAE,fitAE_1,fitAE_2))) %>% as.data.frame() 
) %>% 
...
AEvc = as.data.frame(round(fitAE$US$result,3))%>%
  as.data.frame()
colnames(AEvc) = c("VA_places","VA_faces",
                   "VC_places","VC_faces",
                   "VE_places","VE_faces",
                   "SA_places","SA_faces",
                   "SC_places","SC_faces",
                   "SE_places","SE_faces")
rownames(AEvc) = c("places","faces")
...
rA_mat = fitAE$rA$result%>%as.data.frame()%>%
  mutate(sample = "Sutherland et al. 2020",
         facet = "t-t",
         correlation = "rA")
colnames(rA_mat) = c("places","faces","sample","facet","correlation")
rownames(rA_mat) = c("places","faces")

#genetic correlations with confidence intervals
rA = rbind(
  AECr_ci%>%as.data.frame()%>%rowid_to_column()%>%filter(rowid == 2)%>%rename(trait = "rowid")%>%mutate(trait ="places-faces")
)%>%
  mutate(sample = "Sutherland et al. 2020",
         facet = "t-t",
         correlation = "rA")
...
rE_mat =fitAE$rE$result%>%as.data.frame()%>%
  mutate(sample = "Sutherland et al. 2020",
         facet = "t-t",
         correlation = "rE")
colnames(rE_mat) = c("places","faces","sample","facet","correlation")
rownames(rE_mat) = c("places","faces")

#environmental correlations with confidence intervals
rE = rbind(
  AECr_ci%>%as.data.frame()%>%rowid_to_column()%>%filter(rowid == 10)%>%rename(trait = "rowid")%>%mutate(trait ="places-faces")
)%>%
  mutate(sample = "Sutherland et al. 2020",
         facet = "t-t",
         correlation = "rE")
...
####SAVE###
write_csv(modelComparison,sprintf("%s/%s/02_Sutherland_2020/21_AE_tt_multivariate_Sutherland2020_modelComparison.csv",wdOA,wdOA_output))#save model comparison
write_csv(AEvc,sprintf("%s/%s/02_Sutherland_2020/21_AE_tt_multivariate_Sutherland2020_h2.csv",wdOA,wdOA_output))#save estimates
write_csv(AECis,sprintf("%s/%s/02_Sutherland_2020/21_AE_tt_multivariate_Sutherland2020_CI.csv",wdOA,wdOA_output))#save ci
write_csv(mul_r,sprintf("%s/%s/02_Sutherland_2020/21_AE_tt_multivariate_Sutherland2020_rArE.csv",wdOA,wdOA_output))#save correlations