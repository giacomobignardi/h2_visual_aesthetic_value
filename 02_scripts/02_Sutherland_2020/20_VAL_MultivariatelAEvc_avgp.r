...
#load dataFrames:
BioMetric_Pheno  = read_csv(sprintf("%s/%s/02_Sutherland_2020/05_twin_eb_multivariate.csv", wdOA,wdOA_output))
#load fit sat to compare with saturated model
load(sprintf("%s/%s/02_Sutherland_2020/16_SAT_eb_multivariate_Sutherland2020.RData", wdOA,wdOA_output))

#load functions:
source(sprintf("%s/%s/functions/miFunctions.R", wdOA,wdOA_scripts))


muvTraits = c("SC_A","SC_B","FA_A","FA_B")
...
####SAVE###
write_csv(modelComparison,sprintf("%s/%s/02_Sutherland_2020/20_AE_eb_multivariate_Sutherland2020_modelComparison.csv",wdOA,wdOA_output))#save model comparison
write_csv(AEvc,sprintf("%s/%s/02_Sutherland_2020/20_AE_eb_multivariate_Sutherland2020_h2.csv",wdOA,wdOA_output))#save estimates
write_csv(AECis,sprintf("%s/%s/02_Sutherland_2020/20_AE_eb_multivariate_Sutherland2020_CI.csv",wdOA,wdOA_output))#save ci
write_csv(mul_r,sprintf("%s/%s/02_Sutherland_2020/20_AE_eb_multivariate_Sutherland2020_rArE.csv",wdOA,wdOA_output))#save correlations
