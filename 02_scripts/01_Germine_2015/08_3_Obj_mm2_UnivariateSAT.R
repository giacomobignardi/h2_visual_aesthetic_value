...
set.seed(42)
Obj_BioMetric_Pheno = BioMetric%>%filter(category =="AO")%>%dplyr::select(FamId, Age, Sex, mm2_z_1,mm2_z_2,Zygosity)%>%as.data.frame()
...
fitEMVZ <-mxTryHard( modelEMVZ, intervals=T) #cope with status red
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitSAT,fitCOVsex,fitCOVsex,fitEMO,fitEMO,fitEMVO,fitEMVZ, file = sprintf("%s/%s/01_Germine_2015/08_3_SAT_tt_abstracts_Germine2015.RData",wdOA,wdOA_output))
save(modelComparison,file = sprintf("%s/%s/01_Germine_2015/08_3_SAT_tt_abstracts_modelComparison.RData",wdOA,wdOA_output))
write_csv(fitSATdescriptives,sprintf("%s/%s/01_Germine_2015/08_3_SAT_tt_abstracts_parameters.csv",wdOA,wdOA_output))
write_csv(modelComparison,sprintf("%s/%s/01_Germine_2015/08_3_SAT_tt_abstracts_modelComparison.csv",wdOA,wdOA_output))
