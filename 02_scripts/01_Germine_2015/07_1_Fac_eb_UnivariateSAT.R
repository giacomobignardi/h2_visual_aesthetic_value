...
set.seed(42)
Fac_BioMetric_Pheno = BioMetric%>%filter(category =="FA_TOT")%>%dplyr::select(FamId, Age, Sex, avgP_1,avgP_2,Zygosity)%>%as.data.frame()
# REPARE DATA ----------------------------------------------------------------------------------------------------------------------
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitSAT,fitCOVsex,fitCOVsex,fitEMO,fitEMO,fitEMVO,fitEMVZ, file = sprintf("%s/%s/01_Germine_2015/07_1_SAT_eb_faces_Germine2015.RData",wdOA,wdOA_output))
save(modelComparison,file = sprintf("%s/%s/01_Germine_2015/07_1_SAT_eb_faces_modelComparison.RData",wdOA,wdOA_output))
write_csv(fitSATdescriptives,sprintf("%s/%s/01_Germine_2015/07_1_SAT_eb_faces_parameters.csv",wdOA,wdOA_output))
write_csv(modelComparison,sprintf("%s/%s/01_Germine_2015/07_1_SAT_eb_faces_modelComparison.csv",wdOA,wdOA_output))
