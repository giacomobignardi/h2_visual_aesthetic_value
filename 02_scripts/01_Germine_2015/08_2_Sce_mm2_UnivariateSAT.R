...
set.seed(42)
Sce_BioMetric_Pheno = BioMetric%>%filter(category =="SC")%>%dplyr::select(FamId, Age, Sex, mm2_z_1,mm2_z_2,Zygosity)%>%as.data.frame()

# REPARE DATA ----------------------------------------------------------------------------------------------------------------------
# Select Variables for Analysis
vars <- 'mm2_z' # list of variables names
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitSAT,fitCOVsex,fitCOVsex,fitEMO,fitEMO,fitEMVO,fitEMVZ, file = sprintf("%s/%s/01_Germine_2015/08_2_SAT_tt_scenes_Germine2015.RData",wdOA,wdOA_output))
save(modelComparison,file = sprintf("%s/%s/01_Germine_2015/08_2_SAT_tt_scenes_modelComparison.RData",wdOA,wdOA_output))
write_csv(fitSATdescriptives,sprintf("%s/%s/01_Germine_2015/08_2_SAT_tt_scenes_parameters.csv",wdOA,wdOA_output))
write_csv(modelComparison,sprintf("%s/%s/01_Germine_2015/08_2_SAT_tt_scenes_modelComparison.csv",wdOA,wdOA_output))
