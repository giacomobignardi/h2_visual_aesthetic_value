...
set.seed(42)
Fac_BioMetric_Pheno = BioMetric%>%filter(category =="FA_TOT")%>%dplyr::select(FamId, Age, Sex, mm2_z_1,mm2_z_2,Zygosity)%>%as.data.frame()
...
mzData <- subset(Fac_BioMetric_Pheno, Zygosity==1, c(selVars, covVars))
dzData <- subset(Fac_BioMetric_Pheno, Zygosity==3, c(selVars, covVars))
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitSAT,fitCOVsex,fitCOVsex,fitEMO,fitEMO,fitEMVO,fitEMVZ, file = sprintf("%s/%s/01_Germine_2015/08_1_SAT_tt_faces_Germine2015.RData",wdOA,wdOA_output))
save(modelComparison,file = sprintf("%s/%s/01_Germine_2015/08_1_SAT_tt_faces_modelComparison.RData",wdOA,wdOA_output))
write_csv(fitSATdescriptives,sprintf("%s/%s/01_Germine_2015/08_1_SAT_tt_faces_parameters.csv",wdOA,wdOA_output))
write_csv(modelComparison,sprintf("%s/%s/01_Germine_2015/08_1_SAT_tt_faces_modelComparison.csv",wdOA,wdOA_output))
