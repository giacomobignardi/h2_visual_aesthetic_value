...
#
#Description:
# Twin Univariate Saturated model to estimate means and (co)variances (age and sex) across 2 groups (MZss and DZss)
...
set.seed(42)
Sce_BioMetric_Pheno = BioMetric%>%filter(category =="SC")%>%dplyr::select(FamId, Age, Sex, avgP_1,avgP_2,Zygosity)%>%as.data.frame()
...
mzData <- subset(Sce_BioMetric_Pheno, Zygosity==1, c(selVars, covVars))
dzData <- subset(Sce_BioMetric_Pheno, Zygosity==3, c(selVars, covVars))
...
svVa <- .3 # start value for variance
...
covMZ <- mxMatrix( type="Symm", nrow=ntv, ncol=ntv, free=TRUE, values=valDiag(svVa,ntv), labels=c("vMZ1","cMZ21","vMZ2"),
                   name="covMZ" )
covDZ <- mxMatrix( type="Symm", nrow=ntv, ncol=ntv, free=TRUE, values=valDiag(svVa,ntv), labels=c("vDZ1","cDZ21","vDZ2"),
                   name="covDZ" )
...
# SAVE----------------------------------------------------------------------------------------------------------------------
save(fitSAT,fitCOVsex,fitCOVsex,fitEMO,fitEMO,fitEMVO,fitEMVZ, file = sprintf("%s/%s/01_Germine_2015/07_2_SAT_eb_scenes_Germine2015.RData",wdOA,wdOA_output))
save(modelComparison,file = sprintf("%s/%s/01_Germine_2015/07_2_SAT_eb_scenes_modelComparison.RData",wdOA,wdOA_output))
write_csv(fitSATdescriptives,sprintf("%s/%s/01_Germine_2015/07_2_SAT_eb_scenes_parameters.csv",wdOA,wdOA_output))
write_csv(modelComparison,sprintf("%s/%s/01_Germine_2015/07_2_SAT_eb_scenes_modelComparison.csv",wdOA,wdOA_output))
